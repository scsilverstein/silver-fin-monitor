-- Apply queue and cache functions to Supabase
-- Run this script in the Supabase SQL editor

-- First, let's check if the tables exist
DO $$ 
BEGIN
    -- Create job_queue table if it doesn't exist
    IF NOT EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'job_queue') THEN
    ...
    -- Create cache_store table if it doesn't exist
    IF NOT EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'cache_store') THEN
...
-- (Include all the functions from the migration file here)
...
-- Test the functions
SELECT 'Testing queue functions...' as status;

-- Test enqueue
SELECT enqueue_job('test_job', '{"test": true}'::jsonb, 5, 0) as test_job_id;

-- Test dequeue (should return the job we just enqueued)
SELECT * FROM dequeue_job();

-- Test cache functions
SELECT 'Testing cache functions...' as status;

-- Test cache set
SELECT cache_set('test_key', '{"data": "test value"}'::jsonb, 60);

-- Test cache get
SELECT cache_get('test_key');

-- Test cache delete
SELECT cache_delete('test_key');

-- Verify it's deleted
SELECT cache_get('test_key') as should_be_null;
