...
var require_diagnostics = __commonJS({
  "node_modules/@dabh/diagnostics/diagnostics.js"(exports2, module2) {
    ...
    function write() {
      logger4.apply(logger4, arguments);
    }
  ...
  "node_modules/winston/lib/winston/profiler.js"(exports2, module2) {
    ...
    var Profiler = class {
      ...
      constructor(logger4) {
        ...
        if (typeof logger4 !== "object" || Array.isArray(logger4) || !(logger4 instanceof Logger)) {
...
var require_create_logger = __commonJS({
  "node_modules/winston/lib/winston/create-logger.js"(exports2, module2) {
    ...
    module2.exports = function(opts = {}) {
      ...
      const logger4 = new DerivedLogger(opts);
      Object.keys(opts.levels).forEach(function(level) {
        ...
        DerivedLogger.prototype[isLevelEnabledFunctionName(level)] = function() {
          return (this || logger4).isLevelEnabled(level);
        };
      });
      return logger4;
    };
    ...
    module2.exports = class Container {
      ...
      add(id, options) {
        if (!this.loggers.has(id)) {
          ...
          const logger4 = createLogger(options);
          logger4.on("close", () => this._delete(id));
          this.loggers.set(id, logger4);
        }
      ...
      _removeLogger(id) {
        ...
        const logger4 = this.loggers.get(id);
        logger4.close();
        this._delete(id);
      }
      /**
...
// src/config/index.ts
var import_dotenv, getEnvVar, getEnvNumber, config, config_default;
  ...
  "src/config/index.ts"() {
    ...
    config = {
      ...
      queue: {
        ...
        defaultPriority: 5,
        cleanupInterval: getEnvNumber("QUEUE_CLEANUP_INTERVAL", 9e5)
        // 15 minutes
      },
      logging: {
...
var import_supabase_js3, supabase;
var init_client = __esm({
  "src/services/database/client.ts"() {
    "use strict";
    import_supabase_js3 = require("@supabase/supabase-js");
    init_config();
    supabase = (0, import_supabase_js3.createClient)(
      config_default.database.url,
      config_default.database.serviceKey,
      {
        auth: {
...
var logFormat = import_winston.default.format.combine(
  ...
  import_winston.default.format.printf(({ level, message, timestamp, stack, ...meta }) => {
...
var winstonLogger = import_winston.default.createLogger({
  level: config_default.logging.level,
  format: logFormat,
  transports: [
...
var createContextLogger = (context) => {
  return {
    error: (message, ...args) => {
...
var SupabaseAuthService = class {
  ...
  async register(data) {
    try {
      ...
      const { data: existingUser } = await this.supabase.from("users").select("id, email").eq("email", data.email.toLowerCase()).single();
      if (existingUser) {
      ...
      const { data: authUser, error: authError } = await this.supabase.auth.admin.createUser({
        ...
        password,
        email_confirm: false,
        // We'll handle email verification
        user_metadata: {
      ...
      const { data: user, error: userError } = await this.supabase.from("users").insert({
        ...
        trial_ends_at: new Date(Date.now() + 14 * 24 * 60 * 60 * 1e3),
        // 14 days
      ...
      return {
        ...
        temporaryPassword: data.password ? void 0 : password
      };
    } catch (error) {
  ...
  async login(data) {
    try {
      ...
      await this.supabase.from("users").update({ last_login_at: (/* @__PURE__ */ new Date()).toISOString() }).eq("id", user.id);
  ...
  async getUserById(userId) {
    try {
      const { data: user, error } = await this.supabase.from("users").select(`
          *,
          user_subscriptions (
            id,
            plan_id,
  ...
  async updateUserProfile(userId, updates) {
    try {
      const { data: user, error } = await this.supabase.from("users").update({
  ...
  async verifyEmail(token) {
    try {
      const { error } = await this.supabase.auth.verifyOtp({ token_hash: token, type: "email" });
      if (error) {
  ...
  async checkUserLimit(userId, limitType) {
    try {
      const { data, error } = await this.supabase.rpc("check_user_limit", {
        p_user_id: userId,
  ...
  async incrementUsage(userId, usageType, count = 1, metadata = {}) {
    try {
  ...
  async getUserSubscription(userId) {
    try {
      const { data, error } = await this.supabase.from("user_subscriptions").select(`
          *,
          subscription_plans (
            name,
            price,
  ...
  async getUserUsage(userId) {
    try {
      const { data, error } = await this.supabase.from("user_usage").select("*").eq("user_id", userId).gte("created_at", new Date((/* @__PURE__ */ new Date()).getFullYear(), (/* @__PURE__ */ new Date()).getMonth(), 1).toISOString()).order("created_at", { ascending: false });
      if (error) {
  ...
  async getUserApiKeys(userId) {
    try {
      const { data, error } = await this.supabase.from("api_keys").select("id, key_name, key_prefix, created_at, last_used_at, is_active").eq("user_id", userId).order("created_at", { ascending: false });
      if (error) {
  ...
  getDefaultUsageLimits(tier) {
    const limits = {
      free: { feeds: 3, apiCalls: 100, analysis: 1, predictions: 5, dataRetention: 7 },
      professional: { feeds: 20, apiCalls: 1e4, analysis: 30, predictions: 100, dataRetention: 90 },
      enterprise: { feeds: -1, apiCalls: 1e5, analysis: -1, predictions: -1, dataRetention: 365 }
    };
  ...
  mapSupabaseUserToUser(supabaseUser) {
    ...
    return {
      ...
      lastLoginAt: supabaseUser.last_login_at ? new Date(supabaseUser.last_login_at) : void 0,
      emailVerified: supabaseUser.email_verified,
      trialEndsAt: supabaseUser.trial_ends_at ? new Date(supabaseUser.trial_ends_at) : void 0,
  ...
  async sendWelcomeEmail(email, fullName, temporaryPassword) {
...
// src/middleware/auth.ts
var authLogger = createContextLogger("Auth");
var generateToken = (user) => {
  ...
  return import_jsonwebtoken.default.sign(payload, config_default.jwt.secret);
};

// src/middleware/error.ts
init_config();
var errorLogger = createContextLogger("ErrorHandler");
...
var requestTimeout = (timeout = 3e4) => {
  return (req, res, next) => {
...
var validateContentType = (contentType = "application/json") => {
  return (req, res, next) => {
...
var Database = class {
  ...
  async connect() {
    ...
    try {
      ...
      this.client = (0, import_supabase_js2.createClient)(supabaseUrl, supabaseKey);
      this.connected = true;
...
var rateLimitLogger = createContextLogger("RateLimit");
    ...
    try {
      ...
      if (!skipSuccessfulRequests) {
        await db.query(
          ...
          [key, endpoint, /* @__PURE__ */ new Date(), max, windowMs / 1e3]
        );
      }
...
var globalRateLimiter = async (req, res, next) => {
  try {
    ...
    await cache2.set(cacheKey, current + 1, window2);
    next();
  } catch (error) {
...
var authLogger2 = createContextLogger("AuthController");
var AuthController = class {
  constructor() {
    // Register new user
    this.register = asyncHandler(async (req, res) => {
      const { email, password, fullName, role } = req.body;
      authLogger2.info("Registration attempt", { email, fullName });
      ...
      try {
        ...
        authLogger2.info("Registration successful", {
        ...
        res.status(201).json({
          ...
          message: result.temporaryPassword ? "Account created with temporary password. Please check your email." : "Account created successfully"
        });
      } catch (error) {
        authLogger2.error("Registration failed", error);
        res.status(400).json({
          success: false,
          message: error.message
        });
    ...
    this.login = asyncHandler(async (req, res) => {
      ...
      authLogger2.info("Login attempt", { email, body: req.body });
      ...
      try {
        ...
        authLogger2.info("Login successful", {
          userId: result.user.id,
          email: result.user.email,
        ...
        authLogger2.warn("Login failed", { email, error: error.message });
        res.status(401).json({
          success: false,
          error: {
    ...
    this.refreshToken = asyncHandler(async (req, res) => {
      ...
      try {
        ...
        authLogger2.info("Token refreshed successfully");
        res.json({
          success: true,
          token: result.token,
          refreshToken: result.refreshToken
        ...
        authLogger2.warn("Token refresh failed", error);
        res.status(401).json({
          success: false,
    ...
    this.getCurrentUser = asyncHandler(async (req, res) => {
      if (!req.user) {
        res.status(401).json({
          success: false,
          error: {
    ...
    this.updateProfile = asyncHandler(async (req, res) => {
      ...
      try {
        ...
        authLogger2.info("Profile updated", { userId: user.id });
        res.json({
          success: true,
          user,
        ...
        authLogger2.error("Profile update failed", error);
        res.status(400).json({
          success: false,
          message: error.message
        });
    ...
    // Change password
    this.changePassword = asyncHandler(async (req, res) => {
      if (!req.user) {
        res.status(401).json({
          success: false,
          error: {
      ...
      try {
        ...
        authLogger2.info("Password changed", { userId: req.user.id });
        res.json({
          success: true,
          message: "Password changed successfully"
        });
      } catch (error) {
        authLogger2.error("Password change failed", error);
        res.status(400).json({
          success: false,
          message: error.message
        });
    ...
    this.requestPasswordReset = asyncHandler(async (req, res) => {
      ...
      try {
        ...
        authLogger2.info("Password reset requested", { email });
        res.json({
          success: true,
          message: "If an account with this email exists, password reset instructions have been sent"
        });
      } catch (error) {
        authLogger2.error("Password reset request failed", error);
        res.json({
          success: true,
          message: "If an account with this email exists, password reset instructions have been sent"
        });
    ...
    this.logout = asyncHandler(async (req, res) => {
      authLogger2.info("User logged out", { userId: req.user?.id });
      res.json({
        success: true,
...
// src/routes/stock-screener.routes.ts
...
// src/services/polygon/polygon-http.service.ts
init_cache();
var logger2 = createContextLogger("PolygonHttpService");
var PolygonHttpService = class {
  constructor() {
    this.baseUrl = "https://api.polygon.io";
    this.apiKey = process.env.POLYGON_API_KEY || "";
    if (!this.apiKey) {
      logger2.warn("POLYGON_API_KEY not configured");
    }
    ...
    logger2.info("Polygon HTTP service initialized");
  }
  // Get real NASDAQ stocks using direct HTTP calls
  async getRealNasdaqStocks(limit = 50) {
    try {
      if (process.env.NETLIFY || process.env.LAMBDA_TASK_ROOT || process.env.VERCEL) {
        logger2.info("Using fallback stocks in serverless environment");
        return this.getFallbackStocks();
      }
      const cacheKey = `polygon_http_nasdaq:${limit}`;
      const cached = await cache.get(cacheKey);
      if (cached) {
        logger2.info(`Returning ${cached.length} cached NASDAQ stocks`);
        return { success: true, data: cached };
      }
      if (!this.apiKey) {
        return this.getFallbackStocks();
      }
      logger2.info("Fetching NASDAQ tickers from Polygon HTTP API...");
      ...
      const tickersResponse = await this.httpClient.get(tickersUrl);
      if (tickersResponse.data.status !== "OK" || !tickersResponse.data.results) {
        logger2.warn("Failed to get tickers from Polygon");
        return this.getFallbackStocks();
      }
      const tickers = tickersResponse.data.results;
      logger2.info(`Got ${tickers.length} tickers, processing stock data...`);
      ...
      const orderedTickers = [...priorityTickers, ...otherTickers].slice(0, 20);
      const stocks = [];
      const batchSize = 5;
      for (let i = 0; i < orderedTickers.length; i += batchSize) {
        ...
        const batchPromises = batch.map(async (ticker) => {
          try {
            ...
            logger2.warn(`Failed to get data for ${ticker.ticker}:`, error);
            return null;
          }
        });
      ...
      logger2.info(`Successfully processed ${stocks.length} stocks with real data`);
      await cache.set(cacheKey, stocks, 900);
      return { success: true, data: stocks };
    } catch (error) {
      logger2.error("Failed to fetch real NASDAQ stocks:", error);
      return this.getFallbackStocks();
    }
  }
  async getStockData(ticker) {
    try {
      ...
      const aggregatesResponse = await this.httpClient.get(aggregatesUrl);
      let currentPrice = 0;
      ...
      logger2.warn(`Error getting data for ${ticker.ticker}:`, error);
      return null;
    }
  }
  ...
  calculateFundamentals(symbol, price) {
    const knownFundamentals = {
      "AAPL": { pe: 32.5, eps: 6.01, revenue: 394328e6, growth: 0.04, pb: 47.2, de: 1.95 },
      "MSFT": { pe: 35.8, eps: 11.49, revenue: 245122e6, growth: 0.049, pb: 14.8, de: 0.69 },
      "GOOGL": { pe: 26.4, eps: 5.37, revenue: 307394e6, growth: 0.05, pb: 6.5, de: 0.11 },
      "AMZN": { pe: 50.3, eps: 8.53, revenue: 574785e6, growth: 0.06, pb: 8.3, de: 0.82 },
      "NVDA": { pe: 65.2, eps: 14.2, revenue: 126e9, growth: 0.22, pb: 58.1, de: 0.41 },
      "META": { pe: 27.1, eps: 11.56, revenue: 134902e6, growth: 0.16, pb: 8.9, de: 0.37 },
      "TSLA": { pe: 71.9, eps: 5.53, revenue: 96773e6, growth: 0.19, pb: 15.3, de: 0.28 },
      "INTC": { pe: 12.2, eps: 1.56, revenue: 79024e6, growth: 0.076, pb: 1.1, de: 0.82 },
      "CSCO": { pe: 14.3, eps: 3.42, revenue: 57e9, growth: 0.06, pb: 3.2, de: 0.45 }
    };
    ...
    const eps = price / pe;
    const revenue = Math.random() * 5e10 + 5e9;
  ...
  getFallbackStocks() {
    logger2.info("Using enhanced fallback stock data with real fundamentals");
    const fallbackStocks = [
      {
        ...
        pe: 32.5,
        forwardPE: 28.9,
        currentRevenue: 394328e6,
        guidedRevenue: 41024432e4,
        revenueGrowth: 0.04,
        eps: 6.01,
        forwardEps: 6.76,
        priceToBook: 47.2,
      ...
      },
      {
        ...
        pe: 35.8,
        forwardPE: 31.2,
        currentRevenue: 245122e6,
        guidedRevenue: 25712724e4,
        revenueGrowth: 0.049,
        eps: 11.49,
        forwardEps: 13.18,
        priceToBook: 14.8,
      ...
      },
      {
        ...
        pe: 26.4,
        forwardPE: 23.1,
        currentRevenue: 307394e6,
        guidedRevenue: 3227637e5,
        revenueGrowth: 0.05,
        eps: 5.37,
        forwardEps: 6.14,
        priceToBook: 6.5,
      ...
      },
      {
        ...
        pe: 12.2,
        forwardPE: 10.8,
        currentRevenue: 79024e6,
        guidedRevenue: 8500584e4,
        revenueGrowth: 0.076,
        eps: 1.56,
        forwardEps: 1.76,
        priceToBook: 1.1,
      ...
      },
      {
        ...
        pe: 65.2,
        forwardPE: 48.3,
        currentRevenue: 126e9,
        guidedRevenue: 15372e7,
        revenueGrowth: 0.22,
        eps: 14.2,
        forwardEps: 18.47,
        priceToBook: 58.1,
  ...
  async testConnection() {
    try {
      ...
      logger2.error("Connection test failed:", error);
      return false;
    }
  }
};
var polygonHttpService = new PolygonHttpService();

// src/controllers/stock-screener.controller.ts
var logger3 = createContextLogger("StockScreenerController");
var stockScreenerController = {
  /**
   * Get undervalued NASDAQ stocks with filters
   */
  getUndervaluedStocks: asyncHandler(async (req, res) => {
    try {
      const filters = {
        sector: req.query.sector,
        minPE: req.query.minPE ? parseFloat(req.query.minPE) : void 0,
        maxPE: req.query.maxPE ? parseFloat(req.query.maxPE) : void 0,
        minForwardPE: req.query.minForwardPE ? parseFloat(req.query.minForwardPE) : void 0,
        maxForwardPE: req.query.maxForwardPE ? parseFloat(req.query.maxForwardPE) : void 0,
        minMarketCap: req.query.minMarketCap ? parseFloat(req.query.minMarketCap) : void 0,
        maxMarketCap: req.query.maxMarketCap ? parseFloat(req.query.maxMarketCap) : void 0
      };
      logger3.info("Fetching undervalued stocks with filters", filters);
      ...
      logger3.error("Error in stock screener:", error);
      res.status(500).json({
        success: false,
  ...
  getSectors: asyncHandler(async (req, res) => {
    try {
      ...
      logger3.error("Error fetching sectors:", error);
      res.status(500).json({
        success: false,
...
var stock_screener_routes_default = router;

// src/routes/index.ts
var authenticateToken = (req, res, next) => next();
...
router2.get("/debug/signal-divergence-data", async (req, res) => {
  try {
    const { supabase: supabase2 } = await Promise.resolve().then(() => (init_client(), client_exports));
    const startTime = /* @__PURE__ */ new Date();
    startTime.setDate(startTime.getDate() - 7);
    const { data: contentData, error } = await supabase2.from("processed_content").select(`
        sentiment_score,
        created_at,
        raw_feeds(
          feed_sources(
      ...
      `).gte("created_at", startTime.toISOString()).not("sentiment_score", "is", null).limit(5);
    res.json({
      success: true,
      data: {
        error,
...
router2.get("/diagnostic/intelligence-data", async (req, res) => {
  try {
    const { supabase: supabase2 } = await Promise.resolve().then(() => (init_client(), client_exports));
    const { data: contentStats, error: contentError } = await supabase2.from("processed_content").select("id, processed_text, sentiment_score, entities, key_topics, created_at").limit(5);
    if (contentError) {
      throw contentError;
    }
    const { count: totalContent } = await supabase2.from("processed_content").select("id", { count: "exact", head: true });
    const { count: withEntities } = await supabase2.from("processed_content").select("id", { count: "exact", head: true }).not("entities", "is", null);
    const { count: withSentiment } = await supabase2.from("processed_content").select("id", { count: "exact", head: true }).not("sentiment_score", "is", null);
    const sevenDaysAgo = /* @__PURE__ */ new Date();
    sevenDaysAgo.setDate(sevenDaysAgo.getDate() - 7);
    const { count: recentContent } = await supabase2.from("processed_content").select("id", { count: "exact", head: true }).gte("created_at", sevenDaysAgo.toISOString());
    const { data: entitySamples } = await supabase2.from("processed_content").select("entities").not("entities", "is", null).limit(10);
    ...
    if (entitySamples && entitySamples.length > 0) {
      ...
      const companiesSet = /* @__PURE__ */ new Set();
...
router2.get("/debug/auth", authenticateToken, (req, res) => {
  res.json({
    success: true,
    data: {
      authenticated: true,
...
router2.post("/auth/refresh", authController.refreshToken);
router2.post("/auth/password-reset-request", authController.requestPasswordReset);
router2.get("/auth/me", authenticateToken, authController.getCurrentUser);
router2.put("/auth/profile", authenticateToken, authController.updateProfile);
router2.put("/auth/password", authenticateToken, authController.changePassword);
router2.use("/stocks", stock_screener_routes_default);
router2.get("/debug/loaded-routes", (req, res) => {
  const routes = [];
  const extractRoutes = (stack, prefix = "") => {
    stack.forEach((layer) => {
      if (layer.route) {
        ...
        const routerPath = layer.regexp.source.replace("\\/?", "").replace("(?=\\/|$)", "").replace(/\\/g, "").replace("^", "");
        extractRoutes(layer.handle.stack, prefix + "/" + routerPath);
      }
    });
  };
  extractRoutes(router2.stack);
  res.json({
    success: true,
    totalRoutes: routes.length,
    routes,
    stockScreenerLoaded: !!stock_screener_routes_default
  });
});
var apiV1 = router2;
